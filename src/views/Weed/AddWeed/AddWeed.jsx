...
const AddWeed = () => {
    ...
    const saveRecipe = async () => {
        try {
            await axios.post('/weed/', recipe);
            open('Сорняк успешно добавлен');
    ...
    if (types){
        return(
            <div className={'AddRecipe'}>
                <p className={'AddRecipe__title'}>Добавить сорняк</p>
                    <div style={{display: 'flex', flexDirection: 'column'}}>
                <Input type={'file'} onChange={e => saveImage(e.target.files[0])} />
                <TextField className={'AddRecipe__input'} style={{marginTop: 20}} variant={'outlined'} label={'Название сорняка'} onChange={e => setRecipe({...recipe, name: e.target.value})} />
