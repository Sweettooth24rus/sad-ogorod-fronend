...
const AddGarden = () => {
    ...
    const saveRecipe = async () => {
        try {
            await axios.post('/garden/', recipe);
            open('Сад успешно добавлен');
    ...
    if (types){
        return(
            <div className={'AddRecipe'}>
                <p className={'AddRecipe__title'}>Добавить сад</p>
                    <div style={{display: 'flex', flexDirection: 'column'}}>
                <Input type={'file'} onChange={e => saveImage(e.target.files[0])} />
                <TextField className={'AddRecipe__input'} style={{marginTop: 20}} variant={'outlined'} label={'Название сада'} onChange={e => setRecipe({...recipe, name: e.target.value})} />
                ...
                <TextField className={'AddRecipe__input'} style={{marginTop: 20}}  type={'number'} variant={'outlined'} label={'Площадь'} onChange={e => setRecipe({...recipe, square: e.target.value})} />
                </div>
