...
const EditGreenHouse = () => {
    ...
    const getTypes = async () => {
        ...
        setRecipe((await axios.get(`/greenhouse/${greenhouseId}`)).data);
    }
    ...
    const saveRecipe = async () => {
        try {
            await axios.put(`/greenhouse/${greenhouseId}`, recipe);
            open('Теплица успешно изменена');
    ...
    if (types){
        return(
            <div className={'AddRecipe'}>
                <p className={'AddRecipe__title'}>Изменить теплицу</p>
                    <div style={{display: 'flex', flexDirection: 'column'}}>
                <Input type={'file'} onChange={e => saveImage(e.target.files[0])} />
                <TextField value={recipe.name} className={'AddRecipe__input'} style={{marginTop: 20}} variant={'outlined'} label={'Название теплицы'} onChange={e => setRecipe({...recipe, name: e.target.value})} />
                ...
                <TextField value={recipe.materials} className={'AddRecipe__input'} style={{marginTop: 20}}  variant={'outlined'} label={'Материалы'} onChange={e => setRecipe({...recipe, materials: e.target.value})} />
                </div>
