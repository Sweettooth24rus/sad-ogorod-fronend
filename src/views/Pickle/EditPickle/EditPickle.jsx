...
const EditPickle = () => {
    ...
    const saveRecipe = async () => {
        try {
            await axios.put(`/pickle/${pickleId}`, recipe);
            open('Соленье успешно изменено');
    ...
    if (types){
        return(
            <div className={'AddRecipe'}>
                <p className={'AddRecipe__title'}>Изменить соленье</p>
                <div style={{display: 'flex', flexDirection: 'column'}}>
                <Input type={'file'} onChange={e => saveImage(e.target.files[0])} />
                <TextField value={recipe.name} className={'AddRecipe__input'} style={{marginTop: 20}} variant={'outlined'} label={'Название соленья'} onChange={e => setRecipe({...recipe, name: e.target.value})} />
